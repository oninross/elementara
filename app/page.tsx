    ...
    (attacker: Creature, defender: Creature, baseDamage: number, isCriticalMiss = false): number => {
      ...
      setGameState((prev) => {
        ...
        if (prev.player.activeCreature?.id === defender.id) {
          updatedPlayer.activeCreature = { ...defender, currentHp: Math.max(0, defender.currentHp - finalDamage) }
        } else if (prev.opponent.activeCreature?.id === defender.id) {
          updatedOpponent.activeCreature = { ...defender, currentHp: Math.max(0, defender.currentHp - finalDamage) }
        }
        ...
        updatedPlayer.benchCreatures = updatedPlayer.benchCreatures.map((c) =>
          c.id === defender.id ? { ...c, currentHp: Math.max(0, c.currentHp - finalDamage) } : c,
        )
              ...
              setTimeout(() => {
                setGameState((aftershockState) => {
                  ...
                  if (prev.corruptedPlayer === "player" && updatedPlayer.activeCreature?.id === corruptedCreature.id) {
                    updatedPlayer.activeCreature = { ...corruptedCreature, currentHp: newHp }
                  } else if (
                    prev.corruptedPlayer === "opponent" &&
                    updatedOpponent.activeCreature?.id === corruptedCreature.id
                  ) {
                    updatedOpponent.activeCreature = { ...corruptedCreature, currentHp: newHp }
                  }

                  return { ...aftershockState, player: updatedPlayer, opponent: updatedOpponent }
                })
  ...
  const handleTagOut = useCallback(
    (playerType: "player" | "opponent", benchCreature: Creature) => {
      ...
      setGameState((prev) => {
        ...
        const oldActiveCreature = { ...currentPlayerState.activeCreature, isFaceUp: true, turnsSurvived: 0 } // Reset turns survived when tagged out
        const newActiveCreature = { ...newBenchCreatures[benchIndex], isFaceUp: true, turnsSurvived: 0 } // Reset turns survived when tagged in
        ...
        addToLog(
          ...
          } tagged out ${oldActiveCreature.name} for ${newActiveCreature.name}.`,
        )
        ...
        setTimeout(() => {
          setGameState((prev) => {
            const updatedPlayerActive = prev.player.activeCreature
              ...
              : null
            const updatedPlayerBench = prev.player.benchCreatures.map((c) => ({ ...c, isFaceUp: true }))
            ...
            return {
              ...
              turn: firstPlayer,
              hasRolledThisTurn: false,
              player: { ...prev.player, activeCreature: updatedPlayerActive, benchCreatures: updatedPlayerBench },
              opponent: { ...prev.opponent, activeCreature: updatedOpponentActive },
            }
          })
        }, 3000)
      }, 2000)
  ...
  const handleElementSelection = useCallback(
    (element: Element) => {
      ...
      if (creaturesForSelection.length === 0) {
        addToLog(
          `No ${gameState.selectedGameMode?.id === "set-2" ? "Level 3" : "basic"} ${element} creatures available. Please choose another element.`,
        )
        return
      }
            ...
            {(gameState.gamePhase === "inGame" || gameState.gamePhase === "gameOver") && (
              <div className="relative">
                <ArenaSlot
                  ...
                  isDamaged={opponentActiveCreature ? gameState.damagedCreatures.has(opponentActiveCreature.id) : false}
                ...
                {opponentActiveCreature &&
                  gameState.damageAnimations
                    .filter((anim) => anim.creatureId === opponentActiveCreature.id)
                    .map((anim) => (
                      <div
                        key={anim.timestamp}
                        className="absolute inset-0 flex items-center justify-center pointer-events-none z-50"
                ...
                {gameState.gamePhase === "creatureSelection" && (
                  <div className="flex flex-col items-center gap-4 w-full">
                    <h4 className="text-white text-lg font-semibold text-center">
                      🔥🌊🌱🌬️ Awaken Your First Elementara ({gameState.playerSelectedCreatureIds.length}/
                            ...
                            {selectedCreatureInstancesForDisplay.map((creature, index) => (
                              <div key={index} className="relative">
                                ...
                                <Button
                                  variant="destructive"
                                  size="icon"
                                  className="absolute -top-1 -right-1 h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:h-6 rounded-full text-white bg-red-600 hover:bg-red-700 text-xs flex items-center justify-center"
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4">
                          {elementalTypes.map((element) => {
                            ...
                            return (
                              ...
                              >
                                <span className="text-4xl mb-2">{ElementEmoji}</span>
                      ...
                      <div className="text-white text-lg sm:text-xl leading-relaxed space-y-4 text-left">
                        {gameState.selectedGameMode?.id === "set-2" ? (
                          <>
                            ...
                            <p className="text-lg sm:text-xl">🐉 Start with a fully evolved creature.</p>

                            <p className="text-lg sm:text-xl">📊 Use full HP stats, resistances, and weaknesses.</p>

                            <p className="text-lg sm:text-xl">🎲 Roll 1 die each turn to deal damage.</p>

                            <p className="text-lg sm:text-xl">🏆 First to reduce the opponent's HP to 0 wins.</p>

                            <p className="text-lg sm:text-xl font-semibold mt-6">
                              ⚡ Experience the full power of evolved Elementara!
                            </p>
                          </>
                        ) : (
                          <>
                            ...
                            <p className="text-lg sm:text-xl">🥚 Start with your Stage 1 creature.</p>

                            <p className="text-lg sm:text-xl">⚡ Evolve mid-battle to unlock buffs.</p>

                            <p className="text-lg sm:text-xl">🔄 Tag between creatures at any time.</p>

                            <p className="text-lg sm:text-xl">📊 Use HP, resistances, and weaknesses.</p>

                            <p className="text-lg sm:text-xl">🏆 Last creature standing wins.</p>
                      ...
                      <div className="flex flex-col items-center gap-2 mt-4">
                        <div className="bg-purple-900/80 backdrop-blur-sm rounded-lg px-4 py-2 border border-purple-500">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white font-bold">CORRUPTED DIE</span>
                            <span className="text-2xl">🌀</span>
                          </div>
                          <div className="text-center text-purple-200 text-sm mt-1">
                            {gameState.corruptedTurnsRemaining} turns remaining
                          </div>
                          {gameState.corruptedPlayer && (
                            <div className="text-center text-purple-300 text-xs mt-1">
                              {gameState.corruptedPlayer === "player" ? "Your creature" : "Opponent's creature"} is
                              empowered!
                            </div>
                          )}
                        </div>
            ...
            {(gameState.gamePhase === "inGame" || gameState.gamePhase === "gameOver") && (
              <div className="relative">
                <ArenaSlot
                  ...
                  isDamaged={playerActiveCreature ? gameState.damagedCreatures.has(playerActiveCreature.id) : false}
                ...
                {playerActiveCreature &&
                  gameState.damageAnimations
                    .filter((anim) => anim.creatureId === playerActiveCreature.id)
                    .map((anim) => (
                      <div
                        key={anim.timestamp}
                        className="absolute inset-0 flex items-center justify-center pointer-events-none z-50"
                      ...
                      <div className="mt-3">
                        <div className="w-full bg-gray-300 rounded-full h-3 sm:h-4">
                          <div
                            ...
                            style={{
                              width: `${(gameState.selectedCardForDetails.currentHp / gameState.selectedCardForDetails.maxHp) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
