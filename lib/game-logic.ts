...
export const handleWin = (
  ...
  const updatedPlayerCreatures = playerCreatures.map((creature) => {
    ...
    const newHp = wasKOd
      ...
      : Math.max(deEvolvedCreature.currentHp, Math.floor(deEvolvedCreature.maxHp * 0.75)) // Active healed to 75% (or current if higher)
...
export const generateOpponentCreatures = (winCount: number, gameMode: GameMode): Creature[] => {
